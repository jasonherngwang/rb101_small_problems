=begin
First Run Analysis
- Many of the results are zeroes instead of the expected values.
- In the idf method, integer division is being used instead of float division.
  - Add .to_f in the calc for number_of_documents.


Math
- log(0) is infinity. If no documents contain the term, the result will be 
  0 * inf (if the checked doc contains it but is not included in the total
  document array) or NaN (if neither the checked document or total document
  array contain it).
=end
...
# Diagnosis
# p tf('quantum', document1)
# p tf('quantum', document2)
# p tf('quantum', document3)
# p idf('quantum', documents)